...
@pytest.mark.asyncio
async def test_old_generation_job_remains_visible_and_cancels_through_its_owner(
    tmp_path,
):
    ...
    core = SimpleNamespace(
        tools=SimpleNamespace(get_tool=lambda name: SimpleNamespace(manager=manager)),
        scheduler=None,
    ...
    tasks = RuntimeRoleTasks(
        SimpleNamespace(retained_generations=(old, current)), roles
    )
