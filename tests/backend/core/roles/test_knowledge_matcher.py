...
def test_matching_keeps_all_content_without_a_budget() -> None:
    ...
    matched = RoleKnowledgeMatcher().match(
        RoleKnowledgeBase(enabled=True, entries=entries), "rain"
    )
...
def test_primary_keys_trigger_and_secondary_keys_refine(
    ...
    matched = RoleKnowledgeMatcher().match(
        RoleKnowledgeBase(enabled=True, entries=[entry]), text
    )
...
def test_filters_disabled_and_empty_entries_and_orders_remaining_entries() -> None:
    knowledge = RoleKnowledgeBase(
        ...
        entries=[
            RoleKnowledgeEntry(
                id="disabled", content="hidden", always_active=True, enabled=False
            ),
            RoleKnowledgeEntry(id="empty", content=" \n ", always_active=True),
            RoleKnowledgeEntry(
                id="b", content="B", always_active=True, insertion_order=2
            ),
            RoleKnowledgeEntry(
                id="a", content="A", always_active=True, insertion_order=2
            ),
            RoleKnowledgeEntry(
                id="first", content="first", always_active=True, insertion_order=1
            ),
            RoleKnowledgeEntry(
                id="priority", content="priority", always_active=True, priority=2
            ),
        ],
    )
