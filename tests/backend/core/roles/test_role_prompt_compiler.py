...
def test_compiler_uses_stable_profile_order_and_mood_contract() -> None:
    ...
    assert result.content.index("[role_knowledge]") < result.content.index("[role_response_constraints]")
    assert result.content.index("[role_response_constraints]") < result.content.index("Mood Output Contract")
    assert "Mood Output Contract" in result.content


def test_compiler_expands_identity_in_all_runtime_blocks_without_source_metadata() -> None:
    profile = RoleProfile.from_dict(
        {
            "character": {
                "nickname": "小栞", "profile": "{{char}}的资料", "personality": "{{char}}的性格",
                "behavior_rules": "尊重{{user}}", "response_constraints": "回应{{user}}",
            },
    ...
    result = RolePromptCompiler().compile(
        profile,
        [RoleKnowledgeEntry(content="{{char}}认识{{user}}", raw_source={"extension": "不应注入的原始数据"})],
        role_name="Shiori",
        user_name="小明",
    )
...
def test_sparse_named_role_has_runtime_identity_and_identity_updates(tmp_path):
    ...
    role = store.create_role(name="Mira", role_id="mira", system_prompt="旧兼容字段", profile={})

    assert RolePromptCompiler().compile(role).content == "[role_identity]\nMira"
    renamed = store.update_role("mira", name="Shiori", profile={"character": {"nickname": "小栞", "profile": "{{char}}的资料"}})
