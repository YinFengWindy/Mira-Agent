...
def validate_memory_transition(previous: Config, candidate: Config, workspace: Path) -> None:
    ...
    if old.model != new.model or (old.output_dimensionality or VEC_DIM) != (new.output_dimensionality or VEC_DIM):
        raise MemoryStorageIncompatibleError("Existing memory vectors require an explicit migration before changing embedding model or dimensions")
