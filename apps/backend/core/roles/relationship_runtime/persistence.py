...
class _RelationshipPersistenceMixin:
    ...
    def read_snapshot(self, role_id: str) -> dict[str, Any] | None:
        payload = load_json(
            self.snapshot_path(role_id), default=None, domain="role.relationship"
        )
        if not isinstance(payload, dict):
            return None
        return self._normalize_snapshot_payload(
            role_id=role_id, payload=payload, preserve_error=True
        )

    def read_loneliness_runtime(self, role_id: str) -> dict[str, Any] | None:
        payload = load_json(
            self.runtime_path(role_id), default=None, domain="role.loneliness"
        )
        if not isinstance(payload, dict):
            return None
    ...
    def write_snapshot(self, role_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        normalized = self._normalize_snapshot_payload(
            role_id=role_id, payload=payload, preserve_error=True
        )
