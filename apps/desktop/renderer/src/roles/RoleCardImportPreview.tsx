...
export function RoleCardImportPreviewDialog({
  ...
  return (
    <div className="role-card-import-dialog fixed inset-0 z-40 flex items-center justify-center px-4 py-6">
      ...
      <section className="relative z-[1] grid max-h-[min(88vh,960px)] w-full max-w-[720px] grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden rounded-2xl bg-white shadow-pop" role="dialog" aria-modal="true" aria-label="角色卡导入预览" data-testid="role-card-import-preview">
        <header className="flex items-start justify-between gap-4 border-b border-line-soft px-6 py-5">
          <div className="flex min-w-0 items-center gap-3.5">
            {sourceUrl ? (
              <img className="h-16 w-16 shrink-0 rounded-xl object-cover shadow-soft" src={sourceUrl} alt={`${preview.name ?? "角色卡"}预览`} />
            ) : (
              <div className="grid h-16 w-16 shrink-0 place-items-center rounded-xl bg-surface-soft text-ink-muted">
          ...
          <button className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-ink-muted transition hover:bg-surface-hover hover:text-ink-secondary focus:outline-none" type="button" onClick={onClose} aria-label="关闭角色卡预览" title="关闭角色卡预览">
        ...
        <footer className="flex justify-end gap-2 border-t border-line-soft px-6 py-4">
          <button className="rounded-lg border border-line-soft bg-white px-3.5 py-2 text-sm text-ink-secondary transition hover:bg-surface-hover focus:outline-none" type="button" onClick={onCancel}>取消导入</button>
          <button className="rounded-md border border-white/70 bg-gradient-accent px-3.5 py-2 text-sm font-medium text-ink shadow-soft transition-[filter] hover:brightness-[1.03] focus:outline-none disabled:opacity-40" type="button" disabled={hasUnselectedEmotions(assets, selections)} onClick={onClose}>继续编辑</button>
        </footer>
      </section>
    </div>
  );
