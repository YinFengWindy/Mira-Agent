...
function AssetImage({ asset }: { asset: RoleCardImportAsset }) {
  ...
  ) : <div className="grid h-20 w-20 place-items-center rounded-md bg-surface-soft text-[11px] text-ink-faint" aria-hidden="true">无预览</div>;
}
      ...
      {ordinaryAssets.length ? <div className="flex flex-wrap gap-3">
        {ordinaryAssets.map((asset) => (
          <figure className="m-0 grid w-20 justify-items-center gap-1.5" key={asset.asset_id}>
            ...
            <figcaption className="w-full break-words text-center text-[11px] leading-4 text-ink-muted">{assetLabel(asset.kind)}{asset.name ? ` · ${asset.name}` : ""}</figcaption>
          </figure>
        ))}
      </div> : null}
      {duplicates.map(([name, group]) => (
        <fieldset className="min-w-0 border-0 p-0" key={name}>
          <legend className="mb-2 text-xs font-medium text-ink-secondary">心情 · {name}</legend>
