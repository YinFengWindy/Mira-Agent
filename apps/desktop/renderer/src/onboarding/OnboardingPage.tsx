...
export function OnboardingPage({ controller, windowMaximized }: {
  ...
  return (
    <div className="flex h-screen min-h-0 flex-col bg-[#FAFBFC] text-[#182230]" data-testid="onboarding-page">
      ...
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[#E5E7EB] px-6 py-4 sm:px-10">
        <span className="text-xl font-medium">Shiori</span>
        <button type="button" className="rounded-md px-3 py-2 text-sm text-[#667085] hover:bg-black/5 disabled:opacity-40"
      ...
      {settingsOpen ? <>
        ...
        <div className="mx-auto w-full max-w-[680px]">
          <nav aria-label="首次设置进度" className="mb-9 flex flex-wrap gap-x-6 gap-y-3 text-xs text-[#87918E]">
            {steps.filter((_, position) => index < 0 || position >= index).map((item) => (
              <span key={item.id} aria-current={item.id === step ? "step" : undefined}
                className={cx("flex items-center gap-2", item.id === step && "font-medium text-[#286451]")}>
          ...
          {error ? <div className="mb-6 border-l-2 border-red-300 bg-red-50 px-4 py-3">
            <p role="alert" className="break-words text-sm leading-6 text-red-800">{error}</p>
          ...
          {!step && !error ? <p role="status" className="py-12 text-sm text-[#667085]">正在连接...</p> : null}
