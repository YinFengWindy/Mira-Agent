...
将本插件目录复制到 Shiori 仓库外。准备私有 wheelhouse，其中必须有 `shiori-agent`、`shiori-plugin-testkit`、`shiori-plugin-default-memory` 的 0.1.0 wheel，以及 `shiori-plugin-citation` wheel。这些私有包不发布到 PyPI；其余第三方依赖由包元数据解析。
...
安装不使用 editable。不要设置 `PYTHONPATH` 指向原仓库，也不要复制原仓库 `tests/` 或根 conftest。全部测试使用标准 pytest-asyncio；公共启动 fixture 由显式安装的 testkit 注册。
